...
impl ChangeSet {
    ...
    pub fn from_str(changeset: String, pool: &AttributePool) -> Self {
        ...
        if let Some(match_) = option {
            ...
            let change_sign: i32 = if match_.get(2).unwrap().as_str().eq(">") {
                1
            } else {
                -1
            };
        ...
        for operation in &self.ops.inner {
            match operation.op_code {
                ...
                OperationCode::KEEP => {
                    chars
                        .by_ref()
                        .take(operation.chars as usize)
                        .for_each(|c| assm.push(c));
                }
            }
        }
    ...
    fn zip_next(
        ...
        part: &mut Option<Operation>,
        iter: &mut Peekable<Iter<Operation>>,
    ) {
        if op.is_none() {
    ...
    pub fn zip(
        ...
        while op1.is_some()
            ...
            || iter2.peek().is_some()
        {
            ChangeSet::zip_next(&mut op1, &mut op1part, &mut iter1);
            ChangeSet::zip_next(&mut op2, &mut op2part, &mut iter2);
    ...
    pub fn transform(mut self, mut other: ChangeSet, reverse_insert_order: bool) -> ChangeSet {
        ...
        let list = ChangeSet::zip(
            ...
            |this_op, other_op| {
                ...
                let has_insert = this_op.op_code == OperationCode::INSERT
                    || other_op.op_code == OperationCode::INSERT;
                // KEEPs can be reduced by REMOVEs or extended by INSERTs
                let has_keep = this_op.op_code == OperationCode::KEEP
                    || other_op.op_code == OperationCode::KEEP;
                // REMOVEs can reduce KEEPs other REMOVEs
                let has_remove = this_op.op_code == OperationCode::REMOVE
                    || other_op.op_code == OperationCode::REMOVE;
            ...
            move |this_op, other_op, op_out| {
                match (&this_op, &other_op) {
                    ...
                    if inner1.op_code == OperationCode::INSERT
                        || inner2.op_code == OperationCode::INSERT =>
                        {
                            let left;

                            let this_char: String = inner1.char_bank.chars().take(1).collect();
                            let other_char: String = inner2.char_bank.chars().take(1).collect();
                                ...
                                && this_char.ne(&other_char)
                            {
                                ...
                                left = !reverse_insert_order;
                            }

                            if left {
                                // other op goes first
                                *op_out = Some(Operation::new(
                                    OperationCode::KEEP,
                                    inner2.chars,
                                    inner2.lines,
                                    AttributeList::empty(),
                    ...
                    (Some(inner1), Some(inner2))
                    if inner1.op_code != OperationCode::INSERT
                        && inner2.op_code == OperationCode::REMOVE =>
                        {
                            // block 5
                            this_op.take();
                            other_op.take();
                    ...
                    (Some(inner1), Some(inner2))
                    if inner1.op_code == OperationCode::REMOVE
                        && inner2.op_code == OperationCode::KEEP =>
                        {
                            // block 2 3 5
                    ...
                    (Some(inner1), Some(inner2))
                    if inner1.op_code == OperationCode::KEEP
                        && inner2.op_code == OperationCode::KEEP =>
                        {
                            // block 2 4 5
                            let mut operation = Operation::copy(inner1);
                    ...
                    (Some(inner1), None) => {
                        // block 2 5
                        let operation = Operation::copy(inner1);
                        *op_out = Some(operation);
                        this_op.take();
                        other_op.take();
                    }
                ...
                if let Some(op_out_inner) = op_out {
                    dlen += op_out_inner.deltaLen();
                }
            },
        );
    ...
    pub fn pack(&mut self, pool: &AttributePool) -> String {
        ...
        let mut packed_cs = format!(
            "Z:{old_len}{change_sign}{delta_len}{ops}",
            old_len = old_len,
...
mod test {
    ...
    fn test_apply() {
        fn apply(
            ...
            let pool = pool();
            let set = ChangeSet::from_str(cs.into(), &pool);
